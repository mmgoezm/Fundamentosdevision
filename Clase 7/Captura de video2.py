...
def motion_detection():
    # Inicializa la captura de video (0 es la cámara web predeterminada)
    ...
    Background= None
    ret, frameBasura = cap.read()
    time.sleep(2)  # Espera 2 segundos para tomar el primer frame de referencia
    ret, frame1 = cap.read()
    ret, frame2 = cap.read()
    ...
    while cap.isOpened():
        ...
        lowerred1 = np.array([0, 20, 20])
        upperred1 = np.array([10, 255, 255])
        lowerred2 = np.array([160, 100, 20])
        upperred2 = np.array([190, 255, 255])
        ...
        maskred = cv2.medianBlur(maskred,11)
        ...
        for contourred in contoursred:
            ...
            if cv2.contourArea(contourred) < 2100:
                continue
        ...
        maskob=cv2.dilate(maskred, None, iterations=4)
        maskback=cv2.bitwise_not(maskob)

        Back=cv2.bitwise_and(Background, Background, mask=maskob)
        ob=cv2.bitwise_and(frame1, frame1, mask=maskback)
        cv2.imshow("Imagen 2", ob)
        cv2.imshow("Imagen 1", Back)
        sumimg=cv2.addWeighted(ob,1,Back,1,0)
        cv2.imshow("Imagen con objetos", sumimg)  # Mostrar la imagen con los objetos detectados.
